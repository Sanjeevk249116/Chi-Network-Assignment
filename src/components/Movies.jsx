...
function Movies() {
  const { searchMovie, setSearchMovie,  setArray ,array,setPage} =
...
const handleChangeType=(e)=>{
 if(searchMovie){
  ...
  return (
    <div>
      <h1 style={{textAlign:'center', margin:'30px'}}>Movie Suggestion App</h1>
     <div>
      ...
      <label>
        ...
        />
        <button className="btn"onClick={(e) => handleShow(e)}>Search</button>
      </label>
       ...
       <label >
          Sort by Type:
          <select className="sort-by-type" onChange={(e)=>handleChangeType(e)}>
          <option value="">Select an option</option>
      ...
      {num == 1||array ? (
        ...
        <div style={{display:'flex',justifyContent:'center'}}><h2>Please Search Movies...</h2></div>
      )}
    </div>
  );
}
